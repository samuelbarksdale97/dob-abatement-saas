...
function DashboardContent() {
  ...
  const fetchViolations = useCallback(async () => {
    ...
    setLoading(false);
  }, [search, statusFilter, priorityFilter, propertyFilter, dateFrom, dateTo, needsAttention, sortBy, sortDir, page]);
  ...
  const handleClearFilters = useCallback(() => {
    ...
    setPage(1);
    // Explicitly fetch with no filters — don't rely on dependency chain
    // since some values may already be at their default (no state change = no re-render)
    setLoading(true);
    fetch('/api/violations?sortBy=created_at&sortDir=desc&page=1')
      .then(res => res.json())
      .then(data => {
        setViolations(data.violations || []);
        setTotal(data.total || 0);
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <FilterSidebar
            ...
            onPropertyChange={(v) => { setPropertyFilter(v); setPage(1); }}
            dateFrom={dateFrom}
            onDateFromChange={(v) => { setDateFrom(v); setPage(1); }}
            dateTo={dateTo}
            onDateToChange={(v) => { setDateTo(v); setPage(1); }}
            needsAttention={needsAttention}
            onNeedsAttentionChange={(v) => { setNeedsAttention(v); setPage(1); }}
          />
        </div>
